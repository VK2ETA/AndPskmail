...
public class waterfallView extends ImageView {
	...
	public waterfallView(Context context, AttributeSet attrs) {
		...
		start = (int) ((centerfreq - 500.0) * 2048.0 / RxRSID.RSID_SAMPLE_RATE);
		endP1 = 1 + (int) ((centerfreq + 500.0) * 2048.0 / RxRSID.RSID_SAMPLE_RATE);
		xDim = (endP1 - start) * 2;
		Buffer = new int[xDim * yDim];
		maxvalue = AndPskmail.mysp.getFloat("WFMAXVALUE", (float) 11.0);
	}
	...
	public void onDraw(Canvas canvas) {
		...
		topPos = 10; //fixed (debugging)
		leftPos = (wfCwidth - xDim - 80) / 2; //55 dp for the buttons on the right
		...
		canvas.drawRect((float) leftPos, (float) topPos, (float) leftPos + xDim, (float) topPos + yDim, paint);
		canvas.drawLine((float) leftPos + xDim / 2, (float) topPos - 10, (float) leftPos + xDim / 2, (float) topPos, paint);
		...
		if (Modem.newAmplReady)
		{
			...
			for (int i = start; i < endP1; i++)
			{
				...
				if (green < 0) green = 0;
				blue = (int) (Amplitude > maxvalue * 0.5 ? 512.0 * ((maxvalue - Amplitude) / maxvalue) : 512.0 * (Amplitude / maxvalue));
			...
			//Double the tichness for visibility purposes
